    ...
    <div className="container mx-auto p-4 md:p-6 max-w-7xl">
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className={`flex-1 transition-all ${showPreview ? 'lg:w-2/3' : 'lg:w-full'}`}>
          <Card className="border-slate-300 h-[calc(100vh-10rem)] overflow-y-hidden">
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-2">
                  {sections.map((section, index) => (
                    <div key={section.id} className="flex items-center flex-1">
                      <div
                        className={`flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all ${index === currentSection
                      ...
                      {index < sections.length - 1 && (
                        <div
                          className={`flex-1 h-1 mx-2 transition-all ${index < currentSection ? "bg-green-500" : "bg-gray-300"
                            }`}
                        />
                      )}
                    </div>
                ...
                <div className="flex justify-between">
                  {sections.map((section, index) => (
                    <div
                      key={section.id}
                      className={`text-xs text-center flex-1 ${index === currentSection ? "font-semibold" : "text-gray-500"
                        }`}
                    >
                      {section.title}
                    </div>
                              ...
                              <div className="space-y-4">
                                ...
                                {field.value && field.value.length > 0 ? (
                                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    {field.value.map((imageUrl: string | undefined, index: number) => {
                                          ...
                                          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all flex items-center justify-center">
                                            <Button
                                              ...
                                              onClick={(e) => {
                                                e.preventDefault();
                                                const currentGallery = field.value || [];
                                                field.onChange(currentGallery.filter((_: string | undefined, i: number) => i !== index));
                                              }}
                                            >
